...
def main():
    ...
    adj_path='./garage/'+DATASET+'/'     
    pearson_adj=np.load(adj_path+'pearson_adj.npy')
    num_spatial_order=len(args.hidden_dims)
    st_adj_list= construct_st_adj(construct_multi_order_adj(adj,pearson_adj,num_spatial_order),args.strides)
    log_string(log, 'st_adj loaded!')
    ...
    log_string(log, "The shape of spatial-temporal adjacency matrix: {}".format(st_adj_list.shape))
    ...
    for i in tqdm.tqdm(range(1, args.epochs + 1)):
        ...
        if mvalid_loss <= val_loss_min:
            ...
            if os.path.exists(args.save + "exp_" + str(args.expid) + "_" + str(round(val_loss_min, 2)) + "_best_model.pth"):
                os.remove(args.save + "exp_" + str(args.expid) + "_" + str(round(val_loss_min, 2)) + "_best_model.pth")
            wait = 0
        ...
        np.save('result/'+DATASET+'/valid_mae', his_loss)
    ...
    for t in range(args.horizon):
        ...
        logs = 'Horizon: {:d}, Test MAE: {:.4f}, Test MAPE: {:.4f}, Test RMSE: {:.4f}'
    ...
    logs = 'Average Test Error, Test MAE: {:.4f}, Test MAPE: {:.4f}, Test RMSE: {:.4f}'
